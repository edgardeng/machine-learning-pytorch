...
class MyDataset(Dataset):
    def __init__(self, info_filename, train=True, transform=data_tf, target_transform=None, remove_blank=False,
                 character_file=None, image_fold=None):
        ...
        alphabet += ' '
        dir_image = Path(image_fold) if image_fold else Path(__file__).parent
        ...
        for info_name in self.info_filename:
            with open(info_name) as f:
                ...
                for line in content:
                    ...
                    label = ''.join([alphabet[int(i.strip())] for i in params[1:]])
                    fname = str(dir_image.joinpath(m_image_name))
...
class MyDatasetPro(Dataset):
    def __init__(self, info_filename_txtline=list(), info_filename_fullimg=list(), train=True,
                 txtline_transform=data_tf,
