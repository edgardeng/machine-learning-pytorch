...
import crnn as crnn
import config
from online_test import val_model, valudate_model

# config.imgW = 800
# config.alphabet = config.alphabet_v2
# config.nclass = len(config.alphabet) + 1
config.saved_model_prefix = 'CRNN-1010'
config.train_data_fold = r'E:\BaiduNetdiskDownload\Synthetic_Chinese_String\images'
config.train_infofile = r'D:\workspace\DBNet.pytorch\test_data_part1.txt'
config.val_infofile = r'D:\workspace\DBNet.pytorch\test_data_part1.txt'
config.character_file = r'E:\BaiduNetdiskDownload\Synthetic_Chinese_String\char_std_5990.txt'
...
train_dataset = mydataset.MyDataset(info_filename=config.train_infofile, character_file=config.character_file,
...
test_dataset = mydataset.MyDataset(train=False,
    info_filename=config.val_infofile, transform=mydataset.resizeNormalize((config.imgW, config.imgH), is_test=True),
    character_file=config.character_file, image_fold=config.train_data_fold)
...
def val(net, dataset, criterion, max_iter=100):
    ...
    num_correct, num_all = valudate_model(test_dataset, net, config.cuda, converter)
    accuracy = num_correct / num_all

    print('ocr_acc: %f, use time: %.2f' % (accuracy, time.time() - t))
    if config.use_log:
...
def trainBatch(net, criterion, optimizer):
    ...
    # print('input', image.size(), 'output', preds.size())
